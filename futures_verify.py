#!/usr/bin/env python3
"""
Locked level-fade engine (cond BE@45, 10% trail-after-engagement, TP=SL=
min(1.5x prior completed 1h range, 200), 19:00-11:00 ET entries, 15:00
cutoff, SAL), parameterized by instrument so the exact same trade logic in
nq_verify.py can be run against ES (or any other future/vol-index pair)
without duplicating it.

Usage:
    python3 futures_verify.py nq --bars data/nq_1m_2018_2026.csv.gz --vol data/vxn_daily.csv
    python3 futures_verify.py es --bars data/es_1m_2018_2026.csv.gz --vol data/vix_daily.csv
"""
...
# ── constants (identical to nq_verify.py) ─────────────────────────────────────
...
INSTRUMENTS = {
    "nq": InstrumentParams(sigma_mult=1.25, offset_pct=None, ib_minutes=60, fixed_offset=15.75),
    "es": InstrumentParams(sigma_mult=1.25, offset_pct=0.02, ib_minutes=60, fixed_offset=None),
}
...
def _prior_close(vol: pd.Series, session_date) -> float | None:
...
def generate_levels(bars: pd.DataFrame, vol: pd.Series, params: InstrumentParams) -> pd.DataFrame:
...
# ── session helpers (identical to nq_verify.py) ───────────────────────────────
...
# ── trade simulation (identical to nq_verify.py) ──────────────────────────────
...
def build_trades(bars: pd.DataFrame, vol: pd.Series, params: InstrumentParams):
    levels  = generate_levels(bars, vol, params)
...
def main():
    ...
    ap.add_argument("--vol",  required=True, help="daily vol-index closes CSV (date, close)")
    args = ap.parse_args()

    params = INSTRUMENTS[args.instrument]

    print("loading data...")
    bars = load_1m_ohlcv(args.bars)
    vol  = load_vol_daily(args.vol)
    print(f"  bars: {len(bars):,} rows  {bars.index[0].date()} -> {bars.index[-1].date()}")
    print(f"  vol:  {len(vol):,} days\n")

    print("building trades (pre-SAL)...")
    raw  = build_trades(bars, vol, params)
